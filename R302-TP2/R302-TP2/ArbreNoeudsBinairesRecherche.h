...
private:
    ...
    int getHauteurRecWorker(NoeudBinaire<TypeInfo>* ptrRac) const;
     ...
     bool estPliableRecWorker(const NoeudBinaire<TypeInfo>* ptrRacGauche, const NoeudBinaire<TypeInfo>* ptrRacDroit) const;
...
int ArbreNoeudBinaireRecherche<TypeInfo>::getHauteurRecWorker(NoeudBinaire<TypeInfo>* ptrRac) const {
    if(ptrRac == nullptr){
        ...
        return 1 + max(getHauteurRecWorker(ptrRac->getRefPtrFilsDroit()), getHauteurRecWorker(ptrRac->getRefPtrFilsGauche()));
    }
    return 0;
...
void ArbreNoeudBinaireRecherche<TypeInfo>::insertRecWorker(NoeudBinaire<TypeInfo>*& ptrRac, const TypeInfo& nouvelleInfo) {
    // Cas de base : si ptrRac est nul, on crée un nouveau nœud
    if (ptrRac == nullptr) {
        ptrRac = new NoeudBinaire<TypeInfo>(nouvelleInfo); // Création du nouveau nœud avec la valeur
    } else {
        // Comparaison pour insérer à gauche ou à droite
        if (nouvelleInfo <= ptrRac->getInfo()) {
            // Insérer à gauche si la nouvelle information est inférieure ou égale
            insertRecWorker(ptrRac->getRefPtrFilsGauche(), nouvelleInfo);
        } else {
            // Insérer à droite si la nouvelle information est supérieure
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::estEgalARecWorker(const NoeudBinaire<TypeInfo>* monPtrRac, const NoeudBinaire<TypeInfo>* sonPtrRac) const {
    ...
    return aMemeGeometrieQueRecWorker(monPtrRac->getPtrFilsGauche(), sonPtrRac->getPtrFilsGauche()) &&  aMemeGeometrieQueRecWorker(monPtrRac->getPtrFilsDroit(), sonPtrRac->getPtrFilsDroit()) && (monPtrRac->getInfo() == sonPtrRac->getInfo()); 
...
template<class TypeInfo>
bool ArbreNoeudBinaireRecherche<TypeInfo>::estPliableRecWorker(
    const NoeudBinaire<TypeInfo>* ptrRacGauche, 
    const NoeudBinaire<TypeInfo>* ptrRacDroit) const
{
    // Vérification si les pointeurs sont nuls
    if (ptrRacGauche == nullptr && ptrRacDroit == nullptr) {
        return true;  // Les deux sous-arbres sont vides, donc pliables
    }

    if (ptrRacGauche == nullptr || ptrRacDroit == nullptr) {
        return false;  // L'un des sous-arbres est vide, l'autre non, donc pas pliable
    }

    // Si les pointeurs ne sont pas nuls, on peut accéder à leurs valeurs
    cout << ptrRacGauche->getInfo() << " - " << ptrRacDroit->getInfo() << endl;

    

    // Vérification récursive des sous-arbres opposés
    return estPliableRecWorker(ptrRacGauche->getPtrFilsGauche(), ptrRacDroit->getPtrFilsDroit()) &&
           estPliableRecWorker(ptrRacGauche->getPtrFilsDroit(), ptrRacDroit->getPtrFilsGauche());
}
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::aPourSousArbreRecWorker(const NoeudBinaire<TypeInfo>* ptrRacCetArbre, const NoeudBinaire<TypeInfo>* ptrRacUnArbre) const {
    if (ptrRacUnArbre == nullptr) {
        return true; // Un arbre vide est toujours un sous-arbre.
    }
    else if (ptrRacCetArbre == nullptr) {
        return false; // Impossible de trouver un sous-arbre dans un arbre vide.
    }
    else if(ptrRacCetArbre->getInfo() == ptrRacUnArbre->getInfo() && estEgalARecWorker(ptrRacCetArbre, ptrRacUnArbre)){
                    return true;
    }

    else if (ptrRacCetArbre->getInfo() < ptrRacUnArbre->getInfo()){
        return  aPourSousArbreRecWorker(ptrRacCetArbre->getPtrFilsGauche(), ptrRacUnArbre) || aPourSousArbreRecWorker(ptrRacCetArbre->getPtrFilsDroit(), ptrRacUnArbre);
    }else{
        return aPourSousArbreRecWorker(ptrRacCetArbre->getPtrFilsGauche(), ptrRacUnArbre) || aPourSousArbreRecWorker(ptrRacCetArbre->getPtrFilsDroit(), ptrRacUnArbre);
    }
}
...
bool ArbreNoeudBinaireRecherche<TypeInfo>::estPliable() const {
    // Si l'arbre est vide, il est pliable par définition
    if (ptrRacine == nullptr) {
        return true;
    }
    // Appeler le worker en passant les sous-arbres gauche et droit
    return estPliableRecWorker(ptrRacine->getPtrFilsGauche(), ptrRacine->getPtrFilsDroit());
}
